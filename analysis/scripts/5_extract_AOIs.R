...
lu_AOI_1 <- crop(LIDAR2018_lu_osm_repr_filt, AOI_1)
plot(AOI_1)
...
lu_AOI_2 <- crop(LIDAR2018_lu_osm_repr_filt, AOI_2)
plot(AOI_2)
...
lu_AOI_3 <- crop(LIDAR2018_lu_osm_repr_filt, AOI_3)
plot(AOI_3)
plot(lu_AOI_3, add = TRUE)
#the burial mounds are on the terrain of the Botanicla Garden, thus the class
#recreational is filtered out from this shapfile
...
#test
masked <- mask(x = AOI_3, mask = lu_AOI_3_filt)
writeRaster(masked, paste0(path_tests, "mask_AOI_3.tif"), format = "GTiff",  overwrite=TRUE)
lu_AOI_3_cr <- crop(x = masked, y = extent(AOI_3))
writeRaster(lu_AOI_3_cr, paste0(path_tests, "lu_AOI_3.tif"), format = "GTiff",  overwrite=TRUE)
plot(AOI_3)
plot(lu_AOI_3_cr, add = TRUE)


lu_AOI_4 <- crop(LIDAR2018_lu_osm_repr_filt, AOI_4)
plot(AOI_4)
...
lu_AOI_5 <- crop(LIDAR2018_lu_osm_repr_filt, AOI_5)
plot(AOI_5)
